...
import { articlesAdminAPI } from "../../lib/api";
...
function buildFormData(f, status, existing, user) {
  ...
  fd.append("sections", JSON.stringify(content));
  fd.append("status", status);
  fd.append("featured", String(Boolean(f.trending)));
            ...
            <div key={i} className="ab-card">
              <div className="ab-card-hdr">
                <div className="ab-type">
                  ...
                  <select
                    className="ab-type-sel"
                    value={b.type}
                    onChange={(e) => chtype(i, e.target.value)}
                  >
                    {BTYPES.map((t) => (
  ...
  async function handleSubmit(status) {
    ...
    try {
      const payload = buildFormData(
        {
          ...form,
          author: form.author || user?.fullName || user?.name || "",
          authorRole:
            form.authorRole ||
            user?.subjectLine ||
            user?.title ||
            ...
            user?.industry ||
            "Trainer",
          initials: form.initials || mkInit(user?.fullName || user?.name || ""),
        },
        status,
        article,
        user
      ...
      const res = isEdit
        ? await articlesAdminAPI.update(articleId, payload)
        : await articlesAdminAPI.create(payload);
      ...
      >
        <div className="mbox">
          ...
          <div className="mbody">
            ...
            <div className="msec">
              Cover Image <span className="opt">(required for create)</span>
            </div>
              ...
              <div className="tags-wrap">
                {form.tags.map((t) => (
                  <span key={t} className="tpill">
                    {t}
                    <button type="button" className="tpill-rm" onClick={() => removeTag(t)}>
                      ×
                    </button>
                  </span>
                ))}
