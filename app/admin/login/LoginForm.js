...
export default function LoginForm() {
  ...
  const { user, loading, error, token, initialized } = useSelector((state) => state.auth)
  ...
  useEffect(() => {
    ...
    router.replace(user.role === 'admin' ? '/admin/homepage' : '/trainer/dashboard')
  ...
  const handleSubmit = async (e) => {
    ...
    try { 
      ...
      router.replace(result.user.role === 'admin' ? '/admin/homepage' : '/trainer/dashboard')
