...
const Category = () => {
  ...
  useEffect(()=>{
    const fetchFeaturedWorkshops  = async ()=>{
      try{
        const res = await axios.get(`httpss:/ttptrainer-baokend-production.up.rpitway.app/ckend-production.up.railway.app/eatured-lists?itemType=Workshop&category=${activeFilter}`)
        if(res.data.success){
