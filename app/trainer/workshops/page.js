...
.trainer-subnote{font-size:.76rem;color:#64748b;margin-top:4px;line-height:1.5;}
...
function resolveWorkshopPeople(w) {
  const createdBy = getSafeObject(w?.createdBy);
  const assignedTrainer = getSafeObject(w?.assignedTrainer);
  const creatorType = w?.creatorType || "";

  const isTrainerCreated = creatorType === "TrainerProfile";
  const isAdminCreated = creatorType === "Admin";

  const trainerSource = isTrainerCreated
    ? createdBy
    : assignedTrainer || null;

  const coordinatorSource = isAdminCreated ? createdBy : null;
...
function mapWorkshopFromBackend(w) {
  const people = resolveWorkshopPeople(w);

  const trainer = people.trainerSource
    ? {
        name:
          people.trainerSource.fullName ||
          people.trainerSource.name ||
          "Trainer",
        profileImage:
          people.trainerSource.profilePhoto?.url ||
          people.trainerSource.avatar?.url ||
          people.trainerSource.avatar ||
          "",
        designation:
          people.trainerSource.subjectLine ||
          people.trainerSource.companyName ||
          people.trainerSource.role ||
          "",
        experience:
          people.trainerSource.additionalDetails?.trainingExperience || "",
      }
    : null;

  const coordinator = people.coordinatorSource
    ? {
        name:
          people.coordinatorSource.fullName ||
          people.coordinatorSource.name ||
          "Admin",
        profileImage:
          people.coordinatorSource.profilePhoto?.url ||
          people.coordinatorSource.avatar?.url ||
          people.coordinatorSource.avatar ||
          "",
        designation:
          people.coordinatorSource.role ||
          people.coordinatorSource.companyName ||
  ...
  return {
    _id: w._id,
    creatorType: people.creatorType,
    isAdminCreated: people.isAdminCreated,
    isTrainerCreated: people.isTrainerCreated,
    status: w.status,
    ...
    certifications: [],
    photos: (w.mediaGallery?.snapshots || []).map((s, i) => ({
      src: s.url,
      label: `Session ${i + 1}`,
    })),
    ...
    <div className="w-card" style={{ animationDelay: `${delay}ms` }} onClick={() => onView(w)}>
      <div className="w-cover">
        {w.coverImg ? (
          ...
          <div className="w-cover-ph">
            <BookOpen size={38} style={{ color: "#c4b5fd" }} />
          </div>
        )}
      ...
      <div className="w-body">
        ...
        <div className="w-meta">
          {dur && (
            <span className="w-meta-item">
              <Clock size={11} /> {dur}
            </span>
          )}
          {w.seats ? (
            <span className="w-meta-item">
              <Users size={11} /> {w.seats} seats
            </span>
          ) : null}
          {w.mode && (
            <span className="w-meta-item" style={{ textTransform: "capitalize" }}>
              {w.mode}
            </span>
          )}
        </div>
        ...
        <div className="w-price">
          {w.price?.discounted ? (
            <>
              <span className="w-price-main">{fmt(w.price.discounted)}</span>
              <span className="w-price-orig">{fmt(w.price.original)}</span>
            </>
          ) : w.price?.original ? (
      ...
      <div className="w-foot" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="w-acts">
          <button className="btn-act btn-edit" title="Edit" onClick={() => onEdit(w)}>
            <Pencil size={14} />
          </button>
          <button className="btn-act btn-del" title="Delete" onClick={() => onDelete(w)}>
            <Trash2 size={14} />
          </button>
        </div>
      </div>
...
function WorkshopDetail({ w, onBack, onEdit, onDelete }) {
  ...
  const includes = Array.isArray(w.price?.includes) && w.price.includes.length > 0 ? w.price.includes : [];
  const outcomes = Array.isArray(w.learningOutcomes) && w.learningOutcomes.length > 0 ? w.learningOutcomes : [];
  ...
  if (Array.isArray(w.classTypes) && w.classTypes.length > 0) {
    ...
    if (w.mode === "online" || w.mode === "hybrid") {
      conductModes.push({ icon: <Wifi size={20} />, label: "Live Online", sub: "Online sessions", color: "#2563eb", bg: "#eff6ff" });
    }
    if (w.mode === "offline" || w.mode === "hybrid") {
      conductModes.push({ icon: <MapPin size={20} />, label: "Offline", sub: "In-person", color: "#7c3aed", bg: "#f5f3ff" });
    }
  ...
  const trainer = w.trainer || null;
  const coordinator = w.coordinator || null;

  const sidebarTitle = w.isAdminCreated
    ? trainer
      ? "Assigned Trainer"
      : "Workshop Coordinator"
    : "Your Trainer";
  ...
  const sidebarName =
    (w.isAdminCreated ? trainer?.name || coordinator?.name : trainer?.name) || "Trainer";

  const sidebarAvatar =
    (w.isAdminCreated ? trainer?.profileImage || coordinator?.profileImage : trainer?.profileImage) || "";

  const sidebarRole =
    (w.isAdminCreated ? trainer?.designation || coordinator?.designation : trainer?.designation) || "";

  const sidebarSubnote =
    w.isAdminCreated && coordinator?.name && trainer?.name
      ? `Created by ${coordinator.name}`
      : "";
    ...
    <div className="dw">
        ...
        <div className="dhero-inner">
          <div>
            <div className="dnav">
              ...
              {w.isLive && (
                <span className="chip-live">
                  <span className="chip-live-dot" /> LIVE NOW
                </span>
              )}
            </div>
            ...
            <div className="d-rating-row">
              ...
              {dur && (
                <span className="d-rating-item">
                  <Clock size={12} /> {dur}
                </span>
              )}
              {w.seats > 0 && (
                <span className="d-rating-item">
                  <Users size={12} /> {w.seats} seats
                </span>
              )}
            </div>
          ...
          <div className="dhero-right">
            <div className="dhero-img-wrap">
              ...
              {dur && (
                <span className="img-badge img-badge-dur">
                  <Clock size={11} /> {dur}
                </span>
              )}
              {w.seats > 0 && (
                <span className="img-badge img-badge-seats">
                  <Users size={11} /> {w.seats} seats
                </span>
              )}
            </div>
      ...
      <div className="dbody">
        <div className="action-bar">
          ...
          <button className="ab-edit" onClick={() => onEdit(w)}>
            <Pencil size={13} /> Edit
          </button>
          <button className="ab-del" onClick={() => onDelete(w)}>
        ...
        <div className="dgrid">
          <div>
            ...
            {gallery.length > 0 && (
              <div className="sec">
                <h2 className="sec-title">
                  <span className="sec-title-bar" />
                  <Camera size={16} style={{ marginRight: 4 }} /> Session Snapshots
                </h2>
            ...
            {(w.dateRange || w.timeSlot || w.location) && (
              <div className="sec">
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  {w.timeSlot && (
                    <div className="learn-item">
                      <div className="learn-ico" style={{ background: "linear-gradient(135deg,#d97706,#b45309)" }}>
                  ...
                  {w.location && (
                    <div className="learn-item">
                      <div className="learn-ico" style={{ background: "linear-gradient(135deg,#7c3aed,#5b21b6)" }}>
          ...
          <div className="dsidebar sidebar-sticky">
            <div className="trainer-card">
              <p className="trainer-head">{sidebarTitle}</p>

              <div className="trainer-info">
                {sidebarAvatar ? (
                  <img src={sidebarAvatar} alt={sidebarName} className="trainer-av" />
                ) : (
                  <div className="trainer-av-ph">{sidebarName?.[0] || "T"}</div>
                )}

                <div>
                  <div className="trainer-name">{sidebarName}</div>
                  {sidebarRole && <div className="trainer-role">{sidebarRole}</div>}
                  {trainer?.experience && !w.isAdminCreated && (
                    <div className="trainer-exp">
                      <Award size={12} /> {trainer.experience}
                    </div>
                  )}
                  {sidebarSubnote && <div className="trainer-subnote">{sidebarSubnote}</div>}
                </div>
              </div>
  ...
  return (
    <>
      ...
      <div className="page">
            ...
            <div className="content" style={{ paddingTop: workshops.length === 0 ? 32 : 0 }}>
              {workshops.length > 0 && (
                <div className="filter-bar">
                  ...
                  <span className="filter-count">
                    {visible.length} workshop{visible.length !== 1 ? "s" : ""}
                  </span>
                </div>
              )}
              ...
              {fetchError && !loading && (
                <div className="err-box">
                  ...
                  {fetchError}
                  <button className="btn-retry" onClick={fetchWorkshops}>
                    Retry
                  </button>
                </div>
              )}
                ...
                <div className="w-grid">
                  {visible.length === 0 && workshops.length === 0 && (
                    <div className="empty">
                      ...
                      <button className="btn-empty" onClick={openCreate}>
                  ...
                  {visible.length === 0 && workshops.length > 0 && (
                    <div className="empty">
                      <p style={{ color: "var(--muted)", fontSize: ".9rem" }}>No {filter} workshops.</p>
                    </div>
                  )}
        ...
        {showModal && <WorkshopFormModal workshop={editing} onSave={handleSave} onClose={closeModal} />}
          ...
          <div className="overlay">
            <div className="dialog">
              ...
              <div className="dialog-btns">
                <button className="btn-keep" onClick={() => setDeleting(null)}>
                  Keep It
                </button>
